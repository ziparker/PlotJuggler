...
public:
  ...
  TfMsgParserImpl(const std::string& topic_name, PlotDataMapRef& plot_data)
  ...
  {
    auto GetSeries = [&](const std::string& name)
    {
      return & BaseParser::getSeries(BaseParser::_plot_data, name);
    };

    for(const auto& trans: msg.transforms)
    {
      if( BaseParser::_use_header_stamp )
      {
        timestamp = trans.header.stamp.toSec();
      }

      std::string prefix;
      if( trans.header.frame_id.empty() )
      {
        prefix = fmt::format("{}/{}", BaseParser::_topic_name,
      ...
      series->pushBack({ timestamp, trans.header.stamp.toSec() });
